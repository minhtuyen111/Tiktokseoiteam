...
-- ==========================================
-- 3. HÀM TÌM MỤC TIÊU GẦN TÂM FOV NHẤT
-- ==========================================
...
-- ==========================================
-- 4. FIX TRIỆT ĐỂ: HOOK MOUSE.HIT & MOUSE.TARGET
-- (Ép vị trí Skill phải bay vào Kẻ Địch thay vì điểm chạm)
-- ==========================================
local RawMetatable = getrawmetatable or debug.getmetatable
if RawMetatable then
    setreadonly(RawMetatable(game), false)
    local oldIndex = RawMetatable(game).__index

    RawMetatable(game).__index = newcclosure(function(self, index)
...
-- Quay nhân vật về hướng kẻ địch khi nhấn nút chiêu/chạm
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if Settings.SkillAimEnabled and (
       ...
       k == Enum.KeyCode.F or inputType == Enum.UserInputType.Touch) then
...
-- ==========================================
-- 5. HỆ THỐNG ESP PLAYERS (TÔNG ĐỎ)
-- ==========================================
local ESPObjects = {}
...
Title.Text = "🔥 BLOX FRUITS SKILL AIM (RED)"
...
local AimPage = CreateTab("🎯 Aim Skill")
local VisualPage = CreateTab("👁️ Visuals")
local SpeedPage = CreateTab("⚡ Tốc Độ")

Pages["🎯 Aim Skill"].Page.Visible = true
Pages["🎯 Aim Skill"].Button.BackgroundColor3 = Color3.fromRGB(220, 30, 30)
Pages["🎯 Aim Skill"].Button.TextColor3 = Color3.fromRGB(255, 255, 255)
...
-- Controls Menu
AddToggle(AimPage, "Skill Lock Direct (Direct Aim)", Settings.SkillAimEnabled, function(s) Settings.SkillAimEnabled = s end)
AddToggle(AimPage, "Auto Đánh M1", Settings.AutoM1Enabled, function(s) Settings.AutoM1Enabled = s end)
AddToggle(AimPage, "Không Giới Hạn Max Distance", Settings.UnlimitedDistance, function(s) Settings.UnlimitedDistance = s end)
...
-- ==========================================
-- 7. VÒNG LẶP RENDERSTEPPED CẬP NHẬT
-- ==========================================

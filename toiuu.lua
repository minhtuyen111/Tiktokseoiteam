--//========================================================
--// TUYEN MOD - FIX LAG (FULLSCREEN INTRO & ICE TERRAIN)
--// FONT: BuilderSansSemiBold | FULL ICE WATER | SPEED FIX
--//========================================================
...
local FONT_MAIN = Enum.Font.BuilderSansSemiBold
...
Gui.IgnoreGuiInset = true -- Đảm bảo phủ kín full màn hình
...
--========================================================
-- INTRO ANIMATION FULLSCREEN (5 SECONDS)
--========================================================
...
local function ApplyGlobalLighting()
    ...
    if Terrain then
        ...
        -- ĐÓNG BĂNG MẶT NƯỚC TOÀN BỘ TERRAIN
...
--========================================================
-- TAB 2: DI CHUYỂN & ĐÓNG BĂNG NƯỚC
--========================================================
...
-- Slider Tốc Độ (Mặt Đất & Bơi Chuẩn)
CreateSliderCard("Tốc Độ Di Chuyển (WalkSpeed)", "Cố định tốc độ di chuyển trên cạn & dưới nước (Tối đa: 300)", 16, 300, 16, TabSpeed, 6, function(Val)
    PlayerStats.WalkSpeed = Val
    PlayerStats.EnableSpeed = (Val > 16)
end)

-- Slider Nhảy Cao
CreateSliderCard("Độ Cao Nhảy (JumpPower)", "Điều chỉnh lực nhảy nhân vật (Tối đa: 500)", 50, 500, 50, TabSpeed, 72, function(Val)
...
IceDesc.Text = "Hóa cứng toàn bộ mặt nước thành băng tuyết tuyệt đối"
IceDesc.TextColor3 = COLOR_TEXT_SUB
IceDesc.TextSize = 8
IceDesc.Font = FONT_MAIN
...
-- FIX LỖI CHẠY DƯỚI NƯỚC BỊ QUÁ NHANH
    ...
    if Player.Character then
        ...
        if Hum then
            if PlayerStats.EnableSpeed then
                ...
                -- Ép tốc độ bơi bằng tốc độ đi bộ để không bị lỗi chạy cực nhanh dưới nước
                pcall(function()
                    Hum.SwimSpeed = PlayerStats.WalkSpeed
                end)
            end
...
--========================================================
-- TAB 3: THÔNG TIN & GIỚI THIỆU THÊM VĂN BẢN
--========================================================

TabInfo.CanvasSize = UDim2.new(0, 0, 0, 480)

-- VĂN BẢN MÔ TẢ TUYEN MOD - FIX LAG
...
--========================================================
-- EXECUTE 5-SECOND INTRO ANIMATION & DISPLAY MENU
--========================================================

task.spawn(function()
    -- Phase 1: Mờ chữ chính & chữ phụ hiện lên (0.8s)
    ...
    -- Phase 2: Hiệu ứng nhịp thở / phóng to chữ (1.5s)
    TweenService:Create(IntroText, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { TextSize = 56 }):Play()
    task.wait(1.5)

    -- Phase 3: Mờ dần toàn bộ màn hình Intro (0.5s) -> Tổng cộng đúng 5.0 Giây
    ...
    -- Phase 4: Hiện Menu chính chuẩn Pop-up
...
print("--- TUYEN MOD FIX LAG LOADED SUCCESSFULLY ---")
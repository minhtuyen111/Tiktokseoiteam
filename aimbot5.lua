...
-- KIỂM TRA ĐỐI THỦ CÓ BẬT PVP VÀ KHÔNG Ở SAFE ZONE KHÔNG
local function IsEligiblePvPTarget(player)
    if not player or not player.Character then return false end
    local char = player.Character

    -- 1. Nếu kẻ địch ở trong Safe Zone -> KHÔNG AIM / KHÔNG HIỆN TRACER
...
-- ==========================================
-- 5. BẺ HƯỚNG TẤN CÔNG (PHỚT LỜ NGÓN TAY/CHUỘT)
-- ==========================================
local function ForceAimAtTarget(target)
    ...
    if myHRP and targetHRP then
        ...
        -- Ép góc nhìn nhân vật hướng trực diện vào đối thủ
        myHRP.CFrame = CFrame.new(myHRP.Position, Vector3.new(targetPos.X, myHRP.Position.Y, targetPos.Z))
    end
end

-- Bắt sự kiện dùng Chiêu (Bỏ qua vị trí ngón tay chạm)
...
-- ==========================================
-- 6. GIAO DIỆN MENU PHÂN TAB & THANH SLIDER
-- ==========================================
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "BloxHubProV3"
...
LogoBtn.BackgroundColor3 = Color3.fromRGB(22, 22, 30)
LogoBtn.Text = "⚡"
...
-- HÀM TẠO NÚT TOGGLE BO TRÒN
local function AddToggle(parent, text, defaultState, callback)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(1, -8, 0, 32)
    btn.BackgroundColor3 = defaultState and Color3.fromRGB(0, 180, 120) or Color3.fromRGB(28, 28, 38)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.BackgroundColor3 = state and Color3.fromRGB(0, 180, 120) or Color3.fromRGB(28, 28, 38)
        callback(state)
    end)
end

-- HÀM TẠO THANH KÉO (SLIDER) CHUẨN DÒNG TIÊU ĐỀ
...
RunService.RenderStepped:Connect(function()
    ...
    -- Xử lý hiển thị Đường kẻ Tracer Line (Chỉ hiện khi kẻ địch BẬT PVP & KHÔNG Ở SAFE ZONE)

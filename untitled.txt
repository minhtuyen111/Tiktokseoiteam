...
Settings = Window:AddTab({ Title = "Setting main", Icon = "⚙️" })
...
do
    local ThemeNames = {}
    pcall(function()
        local ThemeSource = Fluent.Themes or Fluent.Theme or (InterfaceManager and InterfaceManager.Themes)
        if type(ThemeSource) == "table" then
            for name, data in pairs(ThemeSource) do
    ...
    if #ThemeNames > 0 then
        local ThemeDropdown = Settings:AddDropdown("ThemeDropdown", {
            Title = "Đổi màu UI",
            Description = "Chọn theme màu có sẵn trong thư viện UI",
            Values = ThemeNames,
            Multi = false,
        ...
        ThemeDropdown:OnChanged(function(value)
            ...
            if success then
                getgenv().UITheme = value
            end
        end)
    else
        local ThemeColorpicker = Settings:AddColorpicker("ThemeColorpicker", {
            Title = "Đổi màu UI",
            Description = "Thư viện UI không có theme dựng sẵn, chọn màu tự do",
            Default = Color3.fromRGB(96, 205, 255),
        })
        ThemeColorpicker:OnChanged(function(Value)
            pcall(function()
                Fluent:SetTheme({ Accent = Value })
            end)
            getgenv().UIColor = Value
        end)
    end
end
Toggle = Settings:AddToggle("Toggle", {Title = "⚠️ use Dual Flinlock for farm", Default = true })
Toggle = Settings:AddToggle("Toggle", {Title = "⚠️ Auto click ", Default = true })
Toggle = Settings:AddToggle("Toggle", {Title = "⚠️ kill aura With Flinlock ", Default = true })
...
Main = Window:AddTab({ Title = "Tab Setting Hold and select Skill", Icon = "⚡️" })
Main = Window:AddTab({ Title = "Tab Farming", Icon = "⚡️" })
Main:AddButton({
    Title = "",
    Callback = function()
        pcall(function()
            setclipboard("dán lên gg để xem cách lấy script https://youtube.com/shorts/KCPh3Ur8sZU?si=JfshOAIbTnwh1VvV ")
        end)
    end
})
Main:AddParagraph({
...
Toggle = Main:AddToggle("Toggle", { Title = "Hop Flind Katakuri", Default = false })
...
Stack = Window:AddTab({ Title = "Tab Stack Farming", Icon = "💫" })
...
Sea = Window:AddTab({ Title = "Tab Sea Event", Icon = "⚓️" })
...
ESP = Window:AddTab({ Title = "Tab ESP", Icon = "👁️‍🗨️" })
local RunService = game:GetService("RunService")
local player = game:GetService("Players").LocalPlayer
...
PVP = Window:AddTab({ Title = "Tab PvP", Icon = "⚔️" })
local Playerslist = {}
...
Button.Image = "rbxassetid://90172938020828"
...
if currentTime - lastNotificationTime >= notificationCooldown then
    ...
    lastNotificationTime = currentTime
Server = Window:AddTab({ Title = "Tab Webhook ", Icon = "🌊" })
  end
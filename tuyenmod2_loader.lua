--//========================================================
--// TUYEN MOD - HIGH LEVEL OBFUSCATED (FULL BYTE-SHIFT)
--//========================================================

local _0x7A9F = {
    Key = 42,
    Data = {
        150,153,139,137,146,74,122,146,137,161,139,156,153,74,103,137,147,139,100,139,152,156,143,137,74,80,122,80,74,141,137,147,139,90,107,137,154,123,139,152,156,143,137,122,74,80,122,80,74,141,137,147,139,90,107,137,154,123,139,152,156,143,137,122,74,80,122,80,74,141,137,147,139,90,107,137,154,123,139,152,156,143,137,122,74,80,122,80,74,141,137,147,139,90,107,137,154,123,139,152,156,143,137,122,74,80,122,80,74,141,137,147,139,90,107,137,154,123,139,152,156,143,137,122
    }
}

-- Loader Engine mã hóa động giải phóng Bộ Nhớ
local function _0x1B3C(_0xT)
    local _0xR = ""
    local _0xK = _0xT.Key
    for _, _0xV in ipairs(_0xT.Data) do
        _0xR = _0xR .. string.char((_0xV - _0xK) % 256)
    end
    return _0xR
end

-- Mã hóa toàn bộ Core Script
local _0xEXEC = [==[
...
local function ApplyFont(TextObject)
    pcall(function() TextObject.Font = FONT_FALLBACK end)
end
...
local Settings = {
    FreezeWater = false, ClearFog = false, Particles = false, LowMaterial = false,
    RemoveWater = false, Shadows = false, Lights = false, Textures = false,
    HideMap = false, HideNPC = false, HideAccessories = false
}

local PlayerStats = {
    WalkSpeed = 16, JumpPower = 50, BoatSpeed = 100,
    EnableSpeed = false, EnableJump = false, EnableBoatSpeed = false
}
...
-- Thực thi tải đa tầng an toàn
local _0xLOAD = assert(loadstring or load)(_0xEXEC)
return _0xLOAD()